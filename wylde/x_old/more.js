...
try {
	Promise.map(albums, item => {
		...
		const writeStream = fs.createWriteStream(`${url_id}.txt`)


		axios(config)
			.then(res => { // cambiar por arrow
				const pictures = res.data.data.pictures

				console.log(pictures.length);
				if (pictures) {
					for (let i = 0; i < pictures.length; i++) {
						const url = pictures[i].full;
						const filename = `${pictures[i].id}.jpg`;
						const filePath = `${url_id}/${filename}`;
...
// para cuando son muchas albunes he imagenes
// descarga todo en fichero txt